...
e.stopPropagation()});$('.menu-item-has-children').on('click',function(){$(this).find('ul').stop().slideToggle()})});$(function(){$(window).on("scroll",function(){if($(window).scrollTop()){$('.header').addClass('header--sticky')}else{$('.header').removeClass('header--sticky')}})});$(function(){const scrollLink=$('nav a[href^="#"], .hero a[href^="#"], .procedures a[href^="#"]').not($('.menu-item-has-children > a'));scrollLink.on('click',function(e){e.preventDefault();if($(this).length){var id=$(this).attr('href');var targetOffset=$(id).position().top;const menuHeight=$('.header').innerHeight();$('html, body').stop().animate({scrollTop:targetOffset-menuHeight},800)}else{$('html, body').stop().animate({scrollTop:0},800)}});$('.menu-item-has-children > a').click(function(e){e.preventDefault()})});$(function(){$('.doubts__container__content__card__body').hide();$('.doubts__container__content__card__header').click(function(){$(this).next().slideToggle();$(this).parent().siblings().children().next().slideUp();return!1})});$(function(){$('.result__row__card p').counterUp({delay:30,time:2000})});/*!
...
if(window.Zepto){window.Zepto.fn.waypoint=createExtension(window.Zepto)}}());/*! Magnific Popup - v1.1.0 - 2016-02-20
...
item.preloaded=!0}}});var RETINA_NS='retina';$.magnificPopup.registerModule(RETINA_NS,{options:{replaceSrc:function(item){return item.src.replace(/\.\w+$/,function(m){return'@2x'+m})},ratio:1},proto:{initRetina:function(){if(window.devicePixelRatio>1){var st=mfp.st.retina,ratio=st.ratio;ratio=!isNaN(ratio)?ratio:ratio();if(ratio>1){_mfpOn('ImageHasSize'+'.'+RETINA_NS,function(e,item){item.img.css({'max-width':item.img[0].naturalWidth/ratio,'width':'100%'})});_mfpOn('ElementParse'+'.'+RETINA_NS,function(e,item){item.src=st.replaceSrc(item,ratio)})}}}}});_checkInstance()}));$(function(){$('.gallery__row').magnificPopup({delegate:'a',type:'image',gallery:{enabled:!0}})});/*!
 * sweetalert2 v7.0.8
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/pl-saude/source/Support/Sender.php';$('#form').submit(function(){$(".contact__content__form__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__content__form__status').append(data);setTimeout(function(){$(".contact__content__form__status").addClass("d-none")},3000)},error:function(){$('.contact__content__form__status').append('Erro')}});return!1})})